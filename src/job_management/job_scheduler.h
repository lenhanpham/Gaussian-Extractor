 ...
 *
 * @section JobResources Usage
 * This structure is populated by JobSchedulerDetector and used throughout
 * the
 * application to make resource allocation decisions that respect
...
/// Returns environment variable value with default fallback.
/// @param name Environment variable name
/// @param default_value Default value if variable not found
/// @return Environment variable value or default
...
/// Returns boolean interpretation of environment variable.
/// @param name Environment variable name
/// @param default_value Default value if variable not found
/// @return Boolean value (supports "true"/"false", "yes"/"no", "1"/"0", "on"/"off")
...
/// Returns long integer value from environment variable.
/// @param name Environment variable name
...
/// Converts scheduler type to human-readable name.
/// @param type SchedulerType to convert
...
inline std::string JobSchedulerDetector::scheduler_name(SchedulerType type)
{
    switch (type)
    {
        ...
        case SchedulerType::LSF:
            return "LSF";
        case SchedulerType::UNKNOWN_CLUSTER:
...
/// Checks if currently running under a job scheduler.
/// @return true if running in scheduled job, false if standalone
