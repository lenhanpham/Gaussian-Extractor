...
CoordExtractor::CoordExtractor(std::shared_ptr<ProcessingContext> ctx, bool quiet)
    : context(ctx), quiet_mode(quiet) {}
...
ExtractSummary CoordExtractor::extract_coordinates(const std::vector<std::string>& log_files) {
    ...
    unsigned int num_threads = calculateSafeThreadCount(context->requested_threads ? context->requested_threads : 0,
                                                       log_files.size(),
        ...
        threads.emplace_back([&]() {
            ...
            while ((index = file_index.fetch_add(1)) < log_files.size()) {
                ...
                try {
                    ...
                    auto [success, status] = extract_from_file(log_files[index], error_msg);

                    {
                        std::lock_guard<std::mutex> lock(results_mutex);
                        summary.processed_files++;
                        if (success) {
                            std::string xyz_file = generate_xyz_filename(log_files[index]);
...
std::pair<bool, JobStatus> CoordExtractor::extract_from_file(const std::string& log_file, std::string& error_msg) {
    try {
        ...
        std::string xyz_file = generate_xyz_filename(log_file);
        ...
        for (int l = start + 5; l < end; ++l) {
            ...
            out << std::left << std::setw(10) << symbol 
                << std::right << std::setw(20) << std::fixed << std::setprecision(10) << x 
                << std::setw(20) << std::fixed << std::setprecision(10) << y
...
std::string CoordExtractor::generate_xyz_filename(const std::string& log_file) {
    ...
    return path.replace_extension(".xyz").string();
}
...
void CoordExtractor::report_progress(size_t current, size_t total) {
    ...
    std::cout << "\rExtracting: " << current << "/" << total << " files ("
