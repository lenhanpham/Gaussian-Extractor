...
extract()
{
	...
	ts=$(echo "scale=8; $ent*$temp/627.5096080305927/1000" | bc 2>/dev/null |  awk '{printf "%f", $0}' | cut -c 1-16) 
	...
	tc=$(echo "scale=8; ($tcwithZPE-$zpe)" | bc 2>/dev/null |  awk '{printf "%f", $0}' | cut -c 1-16)
	...
	ts_from_output=$(echo "scale=8; ($tchlowlevel-$tcglowlevel)" | bc 2>/dev/null |  awk '{printf "%f", $0}' | cut -c 1-16)
	if [ ! -z "$scfEqui" ]; then 
		scf=$scfEqui
	...
	sumHighElecLowTchHartree=$(echo "scale=30; ($scf+$tchlowlevel)" | bc 2>/dev/null |  awk '{printf "%.15f", $0}')	
	sumHighElecLowTcgHartree=$(echo "scale=30; ($scf+$tcglowlevel)" | bc 2>/dev/null |  awk '{printf "%.15f", $0}')
	sumHighElecLowTcgkJmol=$(echo "scale=30; ($scf+$tcglowlevel)*2625.5002" | bc 2>/dev/null |  awk '{printf "%.15f", $0}')
...
#	lf=$(grep "Frequencies" ../$file_name | grep -E -- '-[0-9]+(\.[0-9]+)?' | tail -n 1 |awk '{print $3}')
	...
	if [ -z "$scrf" ]; then
		...
		GibbsFreeEV=$(echo "scale=30; $GibbsFreeHartree*27.211396641308" | bc 2>/dev/null |  awk '{printf "%.6f", $0}')
		GibbsFreekJ=$(echo "scale=30; $GibbsFreeHartree*2625.5002" | bc 2>/dev/null |  awk '{printf "%.6f", $0}')
		EnthalpyHartree=$sumHighElecLowTchHartree 
		phaseCorr="NO"
	else
		### Phase correction term 7.96 kJ/mol = 0.003031803235 au
		GibbsFreeHartree=$(echo "scale=30; ($sumHighElecLowTcgHartree+$GphaseCorr)" | bc 2>/dev/null |  awk '{printf "%.6f", $0}')
		GibbsFreeEV=$(echo "scale=30; $GibbsFreeHartree*27.211396641308" | bc 2>/dev/null |  awk '{printf "%.6f", $0}')
		GibbsFreekJ=$(echo "scale=30; $GibbsFreeHartree*2625.5002" | bc 2>/dev/null |  awk '{printf "%.6f", $0}')
		EnthalpyHartree=$sumHighElecLowTchHartree 
		phaseCorr="YES"
	...
	if [ $totalchar -gt 52 ]; then
		startingCut=$(($totalchar-51))
		file_name=$(printf "$file_name" | cut -c $startingCut-) 	
	fi

	# print out deserved information
	printf "%52s %18s %15s %15s %12s %8s %8s\n" "$file_name" "$GibbsFreekJ" "$GibbsFreeHartree" "$GibbsFreeEV" "$lf" "$askt" "$phaseCorr"
...
awk 'BEGIN { printf "%52s %18s %15s %15s %12s %8s %8s\n", "Output name",  "G kJ/mol",   "G a.u",     "G eV",       "LowFQ",  "Status", "PhCorr"
             printf "%52s %18s %15s %15s %12s %8s %8s\n", "-----------",  "----------", "---------", "----------", "-----",  "------", "------"}' 
}
...
if [ -f "$(basename $(pwd))-Gibss-corrected-with-highElevel.results" ]; then 
	rm $(basename $(pwd))-Gibss-corrected-with-highElevel.results  
fi  
...
printf "Temperature in $lastFile: $lastTemp K. Make sure that temperature has been used in your input.\n" | tee $(basename $(pwd))-Gibss-corrected-with-highElevel.results 
printf "The concentration for phase correction: $CM M or $C mol/m3 \n" | tee -a $(basename $(pwd))-Gibss-corrected-with-highElevel.results 
printf "Last Gibbs free correction for phase changing from 1 atm to 1 M: $lastGphaseCorr au \n" | tee -a $(basename $(pwd))-Gibss-corrected-with-highElevel.results 

prinheader | tee -a $(basename $(pwd))-Gibss-corrected-with-highElevel.results 

extractE | sort -n -k2 2>&1 | tee -a $(basename $(pwd))-Gibss-corrected-with-highElevel.results  
